    ...
    <div class="addDiv">
      <div class="add-content">
        ...
        <el-form ref="form" :rules="formDataRules" :model="form" label-width="80px">
          ...
          <el-form-item prop="mains" label="内容" >
            <el-input type="textarea" v-model="form.mains" :autosize="{ minRows: textMinRows, maxRows: textMaxRows}"
                      style="width: 400px"></el-input>
          </el-form-item>
          <div class="btns">
            <el-button type="success" @click="saveParam">提 交</el-button>
            <el-button type="info" style="margin-left: 25px; " @click="resetForm">取 消</el-button>
          </div>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      formDataRules: {
        ...
        mains: [
          {required: true, message: "请输入新闻公告内容", trigger: "blur"}
        ],
      },
    }
  },
  methods: {
    ...
    realSaveForm() {
      ...
      this.$axios.post(this.$HTTPURL + '/news/saveNews', this.form).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功!',
            type: 'success',
