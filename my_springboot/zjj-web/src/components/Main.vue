<template>
  <div class="c">
    ...
    <div>
      ...
      <el-select v-model="academy" filterable placeholder="请选择学院" style="margin-left: 10px" size="small">
        <el-option
      ...
      <el-table-column prop="roleId" label="角色" width="120">
        <template slot-scope="scope">
          <el-tag :type="scope.row.roleId === 0 ? 'danger' : (scope.row.roleId === 1 ? 'primary': 'success')"
                  disable-transitions>{{ scope.row.roleId === 0 ? '管理员' : (scope.row.roleId === 1 ? '教师' : '学生') }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="phone" label="电话" width="120">
      </el-table-column>
      <el-table-column prop="academy" label="学院" width="180" sortable>
      </el-table-column>
    ...
    <el-dialog
      ...
      <el-form ref="form" :rules="formDataRules" :model="form" label-width="100px">
        ...
        <el-form-item label="学院" prop="academy">
          <el-select v-model="form.academy" placeholder="请选择学院" style="width: 260px">
            <el-option
                v-for="item in academys"
                :key="item.value"
                :label="item.label"
...
export default {
  ...
  methods: {
      ...
      this.$axios.get(this.$HTTPURL + '/user/academyList', {}).then(res => res.data).then(res => {
        if (res.code === 200) {
          for (let i = 0; i < res.data.length; i++) {
            ...
            this.states.push(res.data[i].academy)
          }
          console.log(res.data)
          this.mounted()
    ...
    resetForm() {
      let F5form;
      F5form = {no: "", name: "", age: "", phone: "", password: "", sex: "", academy: ""}
      this.form = F5form
    },
    //新增按钮
    addParam() {
