...
<template>
  <div>
    ...
    <div>
      ...
      <el-input
          placeholder="请输入学生姓名" v-model="name" suffix-icon="el-icon-search" style="margin-left:20px;width: 200px"
          size="small"
          @keyup.enter.native="loadGet"></el-input>
    ...
    <div>
      <el-table :data="tableData"
        ...
        <el-table-column prop="remark" label="申请理由" width="300" sortable>
        </el-table-column>
        <el-table-column prop="jobdate" label="申请时间" width="180" sortable>
        </el-table-column>
        <el-table-column prop="states" label="申请状态" width="180" sortable>
        ...
        <el-table-column prop="ss" label="操作">
          <template slot-scope="scope">
            <el-popconfirm title="是否通过申请？" confirm-button-text='通过'
                           cancel-button-text='不通过' @confirm="updateOk(scope.row.joinjobid)"
                           @cancel="updateNotOk(scope.row.joinjobid)"
                           style="margin-left: 10px">
              <el-button slot="reference" size="small" type="success">审核</el-button>
            </el-popconfirm>
            <el-popconfirm title="确定删除吗？" @confirm="deleteParam(scope.row.joinjobid)" style="margin-left: 10px">
...
export default {
  ...
  methods: {
    //审核通过
    updateOk(id) {
      let form = {joinjobid: id, states: "审核已通过"}
      this.$axios.post(this.$HTTPURL + '/joinjob/updateState', form).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功!',
            type: 'success',
...
// 审核不通过
    updateNotOk(id) {
      let form = {joinjobid: id, states: "审核未通过"}
      this.$axios.post(this.$HTTPURL + '/joinjob/updateState', form).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '操作成功!',
    ...
    //删除信息
    deleteParam(id) {
      this.$axios.post(this.$HTTPURL + '/joinjob/delJoinJob', {"joinjobid": id}).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '删除成功!',
            type: 'success',
    ...
    //加载数据
    loadGet() {
      this.$axios.post(this.$HTTPURL + '/joinjob/joinjobList', {
