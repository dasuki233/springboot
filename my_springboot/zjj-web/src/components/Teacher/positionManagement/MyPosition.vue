      ...
      <el-table :data="tableData"
        ...
        <el-table-column prop="ss" label="操作">
          <template slot-scope="scope">
            ...
            <el-popconfirm title="确定删除吗？" @confirm="deleteParam(scope.row.jobid)" style="margin-left: 10px">
    ...
    <el-dialog
      ...
      <el-form ref="form" :rules="formDataRules" :model="form" label-width="80px">
        ...
        <el-form-item label="岗位介绍" prop="description">
          <el-input type="textarea" v-model="form.description" :autosize="{ minRows: 5, maxRows: 10}"
                    style="width: 400px"></el-input>
        </el-form-item>
        <div class="btns">
          <el-button type="success" @click="saveParam">提交</el-button>
          <el-button type="info" style="margin-left: 25px; " @click="closeView">取消</el-button>
        </div>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    deleteParam(id) {
      console.log(id)
      this.$axios.post(this.$HTTPURL + '/job/delJob', {"jobid": id}).then(res => res.data).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '删除成功!',
            type: 'success',
    ...
    modParam(row) {
      ...
      this.form.counts = row.counts
      this.form.states = row.states
      this.form.jdate = row.jdate
      this.form.overTime = row.overTime
    ...
    //加载数据
    loadGet() {
      this.$axios.post(this.$HTTPURL + '/job/TeacherJob', {
